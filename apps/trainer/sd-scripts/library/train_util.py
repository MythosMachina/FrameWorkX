...
class BucketManager:
    ...
    def select_bucket(self, image_width, image_height):
        ...
        if not self.no_upscale:
            ...
            # Guard against zero height/width from tiny images
            if bucket_width <= 0:
                bucket_width = max(self.reso_steps, 1)
            if bucket_height <= 0:
                bucket_height = max(self.reso_steps, 1)
        ...
        # Avoid division by zero if reso height collapsed
        if reso[1] == 0:
            reso = (reso[0], 1)
